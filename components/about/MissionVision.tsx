...
const MissionVision = () => {
  ...
  return (
    <section className="py-2 pb-10 px-6 md:px-16 relative overflow-hidden bg-white">
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="grid lg:grid-cols-2 gap-8 md:gap-12 items-start">
          <motion.div
            {...fadeInUp}
            transition={{ duration: 0.6 }}
            className="relative bg-navy rounded-[2.5rem] p-10 md:p-14 overflow-hidden shadow-[0_30px_60px_-15px_rgba(11,19,43,0.3)]"
          >
            ...
            <div className="flex flex-wrap gap-3">
              {[
                ...
                <span
                  key={pillar}
                  className="px-5 py-2 bg-white/10 text-white rounded-full text-sm font-bold tracking-wide backdrop-blur-md"
                >
                  {pillar}
                </span>
              ))}
          ...
          <motion.div
            ...
            className="relative bg-charcoal rounded-[2.5rem] p-10 md:p-14 overflow-hidden shadow-[0_30px_60px_-15px_rgba(0,0,0,0.4)] lg:mt-24"
          >
            <h3 className="text-4xl md:text-5xl font-extrabold text-white mb-10 tracking-tight">
            ...
            <div className="space-y-8">
              {visionItems.map((item, index) => (
                <div key={index} className="flex items-start gap-6 group">
                  <span className="text-3xl font-black text-neon/40 group-hover:text-neon transition-colors duration-300 mt-1">
                    {item.number}
                  </span>
                  <div>
